...
test('createPullRequestComment ignores missing metrics', (t) => {
  ...
  t.snapshot(createPullRequestComment({ baseSha, metrics, previousMetrics, title: '', job: '', style: 'graph' }))
})


test('createPullRequestComment supports text style', (t) => {
  const baseSha = '0d22eac17bedb89df6db003fe0f19b5cdb4062cc'
  /* eslint-disable no-magic-numbers */
  const metrics = [
    { value: 33_254.267_163_666_664, units: 'ms', name: 'largeDepsNft' },
  ...
  t.snapshot(createPullRequestComment({ baseSha, metrics, previousMetrics, title: '', job: '', style: 'text' }))
})
